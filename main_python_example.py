...
if clientID!=-1:  #check if client connection successful
    ...
    # create a 2D grid of dummy obstacles with radius 0.1 and set their names
    for i in range(10):
        for j in range(10):
            obstacle_handle = sim.simxCreateDummy(clientID, 0.1, None, sim.simx_opmode_blocking)
            sim.simxSetObjectIntParameter(clientID, obstacle_handle, 1001,
                                          sim.simxPackInts([ord(c) for c in f'Obstacle_{i}_{j}\0']),
