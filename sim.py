...
def simx_opstring(clientID, data, operation_mode):
    """
    Sends an operation string to the server.
    """
    request = bytearray(struct.pack('B', simx_header_size + len(data)))
    request.extend(struct.pack('B', operation_mode))
    request.extend(struct.pack('H', len(data)))
    request.extend(data)

    raw_data = b''.join(struct.pack('B', b) for b in request)
    response = b''
    while len(response) < simx_header_size:
        data = simxCallScriptFunction(clientID, "remoteApiCommandServer", sim_scripttype_childscript,
                                          "executeCode_function", [], [], [raw_data], "", simx_opmode_oneshot_wait)
        response += data[2]

    header = struct.unpack('BBBBBBB', response[:simx_header_size])
    data_len = struct.unpack('H', response[simx_header_size:simx_header_size + 2])[0]

    return response[simx_header_size + 2:simx_header_size + 2 + data_len]

def simxGetLastErrorCode(clientID):
    """
    Returns the last error code.
    """
    return simxGetLastErrorCode(clientID)

def simxGetLastErrorMsg(clientID):
    """
    Returns the last error message.
    """
    return simxGetLastErrorMsg(clientID)

def simx_return_error_handling():
    """
    Returns a tuple with error code and message.
    """
    error_code = simxGetLastErrorCode()
    error_message = simxGetLastErrorMsg()
    return error_code, error_message


def simxCreatePureShape(clientID, shape_type, size, mass, options, operation_mode):
    """
    Creates a pure shape object in the scene.
    """
    if isinstance(size, list) or isinstance(size, tuple):
        size = bytearray(struct.pack('fff', *size))

    if options is not None:
        options = bytearray(options)

    request = bytearray(struct.pack('B', simx_header_size + 20 + len(size) + len(options)))
    request.extend(struct.pack('B', simx_opmode_oneshot_wait))
    request.extend(struct.pack('H', 20 + len(size) + len(options)))
    request.extend(struct.pack('B', simx_header_cmd_start))
    request.extend(struct.pack('B', simx_cmd_create_pure_shape))
    request.extend(struct.pack('I', clientID))
    request.extend(struct.pack('I', shape_type))
    request.extend(struct.pack('I', len(size)))
    request.extend(struct.pack('d', mass))
    request.extend(struct.pack('I', len(options)))
    request.extend(size)
    request.extend(options)
    request.extend(struct.pack('B', simx_header_cmd_stop))

    response = simx_opstring(clientID, request, operation_mode)
    if response:
        res, object_handle = struct.unpack('II', response[:8])
        return res, object_handle
    else:
        return simxCallScriptFunction()


sim_boolparam_show_infotext = 28

def sim_boolparam_static(param):
    """
    Sets the state of the 'Static' scene parameter. If param is True, the scene is made static, meaning that
    most non-scripting commands affecting the scene will be ignored. If param is False, the scene is made
    dynamic again. Note that if the scene is static, the 'do not show again' message in dialogs will not be
    shown (since the corresponding dialog would not have any effect on the static scene anyway).
    """
    cmd = simxPackInts([param])
    simxCallScriptFunction('simConst.scene_static', sim_scripttype_customizationscript, 'simBoolParam_callback', cmd, '', '', '', simx_opmode_blocking)
